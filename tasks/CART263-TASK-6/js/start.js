...
let circle = new CircularObj(150,150,20,"#FFC300","#E6E6FA", drawingBoardA.context);
drawingBoardA.addObj(circle)
drawingBoardA.display();

window.addEventListener("click", function(){
    let radius = Math.random() * 10;
    let x = circle.x - Math.cos(circle.angle * 10) * circle.orbitRadius;
    let y = circle.y - Math.sin(circle.angle * 10) * circle.orbitRadius;
    let randomF = '#' + Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0');
    let randomS = '#' + Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0');

    let newCircle = new CircularObj(x,y,radius, randomF,randomS, drawingBoardA.context);
    drawingBoardA.addObj(newCircle);
})

window.addEventListener("keydown", function(e) {
  if (e.code === "Space") {
    // Animates circles out one at a time
...
drawingBoardB.addObj(new RectangularObj(150,10,50,70,"#FF5733","#E6E6FA",drawingBoardB.context))
...
function animationLoop(){
    if (analyser) {
        ...
        let sum = 0;
        // Add up all the audio frequencies
        for (let i = 0; i < dataArray.length; i++) {
            sum += dataArray[i];
        }
        // Calculate the average volume (usually a number between 0 and 150)
        window.micVolume = sum / dataArray.length; 
    }
